...
Output:
- generateGameQuestions function in geminiService.ts with helper and GeminiProvider wrapper
...
<task type="auto">
  <name>Task 1: Implement generateGameQuestions in Gemini service and provider</name>
  <files>services/geminiService.ts, services/providers/geminiProvider.ts</files>
  <action>
**In geminiService.ts:**
...
2. Add helper function for Millionaire progression rules (at end of file, after existing quiz generation functions):
```typescript
...
3. Add main generateGameQuestions function (after the helper):
```typescript
...
4. Add import for new function (in the import block from geminiService):
```typescript
  generateGameQuestions as geminiGenerateGameQuestions,
```

5. Add import for new types (in the import block from aiProvider):
```typescript
import { AIProviderInterface, AIProviderError, USER_ERROR_MESSAGES, GenerationInput, GameQuestionRequest } from '../aiProvider';
```

6. Add new method to GeminiProvider class (after generateQuestionWithAnswer):
```typescript
...
Note: The function returns empty array on error - the caller (Plan 04) will handle retries.
  </action>
  <verify>
1. TypeScript compiles: `npx tsc --noEmit`
2. Function is exported: `grep -q 'export const generateGameQuestions' services/geminiService.ts && echo "OK"`
3. Provider has method: `grep -q 'generateGameQuestions.*GameQuestionRequest' services/providers/geminiProvider.ts && echo "OK"`
  </verify>
  <done>
