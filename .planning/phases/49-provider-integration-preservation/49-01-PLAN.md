...
must_haves:
  truths:
    - "Claude provider detects preservable content from input text"
    - "System prompt includes preservation rules when content is detected"
    - "System prompt includes teleprompter preservation rules for delivery context"
...
<objective>
Integrate content preservation into the Claude provider so detected questions and activities appear verbatim on slides with delivery context in the teleprompter.

Purpose: Teacher-specified content (questions like "What is 3/4 of 12?") must appear exactly as written, not generalized to "practice fractions." This preserves teacher intent.

Output: Modified ClaudeProvider with detection at entry point and preservation rules injected into system prompts for all three generation modes.
</objective>
...
Add two helper functions BEFORE the `getSystemPromptForMode` function:
...
These encapsulate the mode-specific decisions from CONTEXT.md.
  </action>
  <verify>
...
<task type="auto">
  <name>Task 2: Modify getSystemPromptForMode to accept PreservableContent</name>
  <files>services/providers/claudeProvider.ts</files>
  <action>
Modify the `getSystemPromptForMode` function signature to accept an optional fourth parameter:
...
Inject `${preservationRules}` into each mode's system prompt AFTER `${studentFriendlyRules}` and BEFORE the mode-specific rules (e.g., "CRITICAL: You will be provided...").
...
Apply the same pattern to 'refine' and 'blend' modes.
  </action>
  <verify>
Run `npx tsc --noEmit` - function signature change compiles. Check that template strings include the new interpolations.
  </verify>
  <done>
getSystemPromptForMode accepts optional PreservableContent. Preservation rules injected into all three mode prompts.
  </done>
</task>
...
<task type="auto">
  ...
  <files>services/providers/claudeProvider.ts</files>
  <action>
In the `generateLessonSlides` method of ClaudeProvider, after the input normalization block and BEFORE the `const systemPrompt = getSystemPromptForMode(...)` line, add detection:
...
Modify the existing `getSystemPromptForMode` call to pass the detected content:
...
```
  ...
  <verify>
Run `npx tsc --noEmit` - compiles without errors. The detection is called and passed to the prompt builder.
  </verify>
  <done>
Detection happens at entry point. Detected content is passed to system prompt builder.
  </done>
</task>
...
2. Grep for key patterns:
   - `grep -n "detectPreservableContent" services/providers/claudeProvider.ts` (should show import and usage)
   - `grep -n "getPreservationRules" services/providers/claudeProvider.ts` (should show import and usage)
   - `grep -n "getTeleprompterPreservationRules" services/providers/claudeProvider.ts` (should show import and usage)
3. Verify helper functions exist:
   - `grep -n "getDetectionSource" services/providers/claudeProvider.ts`
   - `grep -n "getMinConfidenceForMode" services/providers/claudeProvider.ts`
</verification>

<success_criteria>
- ClaudeProvider imports and uses Phase 48 detection/rules modules
...
- Preservation rules injected into system prompt when content detected
- Teleprompter preservation rules injected for delivery context
- TypeScript compiles without errors
</success_criteria>

<output>
After completion, create `.planning/phases/49-provider-integration-preservation/49-01-SUMMARY.md`
</output>
