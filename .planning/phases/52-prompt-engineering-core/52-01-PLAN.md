...
NOTE: Answer bullet shows definition ONLY, does NOT repeat the vocabulary word (locked decision from CONTEXT.md).
...
- Format guidance (brief, actionable prompts)
  ...
  <verify>
File exists at `services/prompts/teachableMomentRules.ts` with all 5 template constants defined.
Each template includes example question prompts and timing cues.
  </verify>
  <done>
Five scaffolding template constants exist with content-specific strategies and example prompts.
  </done>
</task>
...
All three patterns should be found.
  ...
  <done>
`getTeachableMomentRules` function exists, imports TeachableMoment type, returns empty string for empty array, builds rules with bullet structure, examples, scaffolding templates, and flow guidance.
  </done>
</task>
...
5. Function returns empty string for empty array
6. Function returns rules string for non-empty array
</verification>
