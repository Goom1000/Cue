...
**IMPORTANT DESIGN NOTE:** FinalChaseRound uses a COMPONENT-LOCAL `FinalPhase` type for internal phase management (intro, contestant-round, transition, chaser-round, pushback-opportunity, complete). This is INTENTIONAL and does NOT conflict with the global `ChasePhase` type in types.ts. The orchestrator (TheChaseGame) only needs to know 'final-chase-contestant' or 'final-chase-chaser' - the sub-phases within are UI-only concerns managed locally by this component. When FinalChaseRound completes, it calls onComplete() which the orchestrator uses to transition to 'game-over'.
...
// NOT exposed to global game state - the orchestrator sees 'final-chase-contestant'
// or 'final-chase-chaser' in ChasePhase. These sub-phases are UI-only.
type FinalPhase =
